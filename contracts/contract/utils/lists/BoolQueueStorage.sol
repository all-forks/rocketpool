...
/// @title Bool queue storage helper for RocketStorage data (ring buffer implementation)
/// @author Jake Pospischil
contract BoolQueueStorage is RocketBase {
    ...
    /// @dev The number of items in a bool queue
    ...
    /// @dev The item in a bool queue by index
    function getQueueItem(bytes32 _key, uint _index) external view returns (bool) {
        ...
        return rocketStorage.getBool(keccak256(abi.encodePacked(_key, "item", index)));
    }


    /// @dev The index of an item in a bool queue
    ...
    /// @dev Add an item to the end of a bool queue
    /// @dev Requires that the queue is not at capacity
    /// @dev Requires that the item does not exist in the queue
    function enqueueItem(bytes32 _key, bool _value) onlyLatestRocketNetworkContract external {
        ...
        rocketStorage.setBool(keccak256(abi.encodePacked(_key, "item", index)), _value);
    ...
    /// @dev Remove an item from the start of a bool queue
    ...
    function dequeueItem(bytes32 _key) onlyLatestRocketNetworkContract external {
        ...
        uint start = rocketStorage.getUint(keccak256(abi.encodePacked(_key, "start")));
        bool item = rocketStorage.getBool(keccak256(abi.encodePacked(_key, "item", start)));
        start = start.add(1);
    ...
    /// @dev Remove an item from a bool queue
    ...
    function removeItem(bytes32 _key, bool _value) onlyLatestRocketNetworkContract external {
        ...
        if (index != lastIndex) {
            bool lastItem = rocketStorage.getBool(keccak256(abi.encodePacked(_key, "item", lastIndex)));
