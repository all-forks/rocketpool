...
/// @title Uint queue storage helper for RocketStorage data (ring buffer implementation)
/// @author Jake Pospischil
contract UintQueueStorage is RocketBase {
    ...
    /// @dev The number of items in a uint queue
    ...
    /// @dev The item in a uint queue by index
    function getQueueItem(bytes32 _key, uint _index) external view returns (uint) {
        ...
        return rocketStorage.getUint(keccak256(abi.encodePacked(_key, "item", index)));
    }


    /// @dev The index of an item in a uint queue
    ...
    /// @dev Add an item to the end of a uint queue
    /// @dev Requires that the queue is not at capacity
    /// @dev Requires that the item does not exist in the queue
    function enqueueItem(bytes32 _key, uint _value) onlyLatestRocketNetworkContract external {
        ...
        rocketStorage.setUint(keccak256(abi.encodePacked(_key, "item", index)), _value);
    ...
    /// @dev Remove an item from the start of a uint queue
    ...
    function dequeueItem(bytes32 _key) onlyLatestRocketNetworkContract external {
        ...
        uint start = rocketStorage.getUint(keccak256(abi.encodePacked(_key, "start")));
        uint item = rocketStorage.getUint(keccak256(abi.encodePacked(_key, "item", start)));
        start = start.add(1);
    ...
    /// @dev Remove an item from a uint queue
    ...
    function removeItem(bytes32 _key, uint _value) onlyLatestRocketNetworkContract external {
        ...
        if (index != lastIndex) {
            uint lastItem = rocketStorage.getUint(keccak256(abi.encodePacked(_key, "item", lastIndex)));
