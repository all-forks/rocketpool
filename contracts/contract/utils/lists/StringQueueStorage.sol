...
/// @title String queue storage helper for RocketStorage data (ring buffer implementation)
/// @author Jake Pospischil
contract StringQueueStorage is RocketBase {
    ...
    /// @dev The number of items in a string queue
    ...
    function enqueueItem(bytes32 _key, string memory _value) onlyLatestRocketNetworkContract public {
        ...
        rocketStorage.setString(keccak256(abi.encodePacked(_key, "item", index)), _value);
    ...
    /// @dev Remove an item from the start of a string queue
    ...
    function dequeueItem(bytes32 _key) onlyLatestRocketNetworkContract external {
        ...
        uint start = rocketStorage.getUint(keccak256(abi.encodePacked(_key, "start")));
        string memory item = rocketStorage.getString(keccak256(abi.encodePacked(_key, "item", start)));
        start = start.add(1);
    ...
    function removeItem(bytes32 _key, string memory _value) onlyLatestRocketNetworkContract public {
        ...
        if (index != lastIndex) {
            string memory lastItem = rocketStorage.getString(keccak256(abi.encodePacked(_key, "item", lastIndex)));
