...
contract RocketSettings is RocketBase {
    ...
    function init() public onlyOwner {
        ...
        if (!rocketStorage.getBool(keccak256("settings.init"))) {
            ...
            setUserDepositMax(12 ether);                                                        // Max allowed deposit in Wei 
            setUserWithdrawalAllowed(true);                                                     // Are withdrawals allowed?
            setUserWithdrawalMin(0);                                                            // Min allowed to be withdrawn in Wei, 0 = all
            setUserWithdrawalMax(64 ether);                                                     // Max allowed to be withdrawn in Wei     
            ...
            setMiniPoolBackupCollectTime(1 days);                                             // The time limit of which after a deposit is received back from Casper, that the user backup address can get access to the deposit
